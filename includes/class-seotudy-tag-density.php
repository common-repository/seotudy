...
class Seotudy_tag_density {
	 ...
	 * @since    1.0.0
	 */
	function oneWord($html,$enaz,$tekrar){
		
		$html = $string = html_entity_decode($html, ENT_QUOTES); //html kodlarını htmle çevirdik
		$html = preg_replace("/\W|_/u", " ", $html); //Noktalama işaretlerini kaldırdık
		while(mb_strpos($html,'  ',0,'utf8')!==false) $html = str_replace("  ", " ", $html);//fazla boşlukları sildik

		$html = explode(" ",$html); //Bütün kelimeleri diziye aktardık
		
		$keywords = array();
		//$html = array_unique($html); //Aynı olanları sildik
		$html = array_count_values($html);
		
		foreach($html as $d => $s){
			if(!empty($d) and mb_strlen($d,'utf8')>=$enaz and $s>=$tekrar and !is_numeric($d)){
				$keywords[trim(strip_tags(html_entity_decode($d)))] = (string) trim(strip_tags(html_entity_decode($s)));
			}
		}
		
		arsort($keywords); //En çok tekrarlanandan en aza sıraladık
		//$keywords = array_diff($keywords,$this->common_words);
		return $keywords;
	}
	
	function twoWord($html,$enaz,$tekrar){
		...
		$keywords=array();
		while (list($key, $val) = @each($new_text)){
			
			$tmp = $val;
			list($key, $val) = @each($new_text);
			$tmp = (string) $tmp." ".$val;
			$keywords[] = $tmp;	
		...
		$lastkeywords = array();
		foreach($keywords as $d => $s){
			if(!empty($d) and mb_strlen($d,'utf8')>=$enaz and $s>=$tekrar){
				$lastkeywords[trim(strip_tags(html_entity_decode($d)))] = $s;
			}
		}
	...
	function threeWord($html,$enaz,$tekrar){
